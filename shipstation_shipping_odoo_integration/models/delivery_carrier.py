...
class DeliveryCarrier(models.Model):
    ...
    def shipstation_rate_shipment(self, order):
        ...
        if self.weight_uom == "pounds" and uom_id.name == 'lbs':
            total_weight = round(weight, 3)
        elif self.weight_uom == "pounds" and uom_id.name == 'kg':
            total_weight = round(weight * pound_for_kg, 3)
        elif self.weight_uom == "ounces" and uom_id.name == 'lbs':
            total_weight = round(weight * ounce_for_lb, 3)
            ...
            total_weight = round(weight * grams_for_lb, 3)
    ...
    def generate_label_from_shipstation(self, picking, package_id=False, weight=False):
        ...
        package_id = package_id and package_id.packaging_id or self.delivery_package_id
        # pound_for_kg = 2.20462
        # ounce_for_kg = 35.274
        # grams_for_kg = 1000
        # if self.weight_uom == "pounds":
        ...
        request_data = {
            ...
            "serviceCode": "%s" % (
                    self.shipstation_delivery_carrier_service_id and self.shipstation_delivery_carrier_service_id.service_code),
            "packageCode": "%s" % (self.delivery_package_id.package_code or ""),
            "confirmation": self.confirmation or "none",
            ...
            "dimensions": {
                "units": self.shipstation_dimentions or "inches",
                "length": package_id.length or 0.0,
                "width": package_id.width or 0.0,
                "height": package_id.height or 0.0
            },
            "shipFrom": {
